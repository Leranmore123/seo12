...
echo "=== Testing OAuth /v2/user/info for all 20 accounts ===\n";

foreach ($mappedAccounts as $n => $acc) {
    $url = "https://api.tumblr.com/v2/user/info";
    $authHeader = getTumblrOAuthHeader($acc['key'], $acc['secret'], $acc['token'], $acc['tsecret'], $url, 'GET', []);
    ...
    if ($code === 200) {
        ...
        $userBlogs = array_column($json['response']['user']['blogs'] ?? [], 'name');
        echo "✅ SUCCESS 200: pd{$n} ({$acc['blog']}) | User blogs: " . implode(', ', $userBlogs) . "\n";
    } else {
        echo "❌ FAIL {$code}: pd{$n} ({$acc['blog']}) | " . substr($resp, 0, 70) . "\n";
    }
}
