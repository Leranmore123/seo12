...
// 20 Tumblr Accounts provided by User
$tumblrBlogs = [
...
echo "=== Syncing Tumblr Credentials for pd1..pd42 Users ===\n\n";
...
for ($i = 1; $i <= 42; $i++) {
    ...
    $uStmt->execute([$uname, "{$uname}@gmail.com"]);
    ...
    // Pick mapped Tumblr blog (1 to 20 cyclic)
    $blogIndex = (($i - 1) % 20) + 1;
    $data = $tumblrBlogs[$blogIndex];
    $blog = $data['blog'];
    $apiKey = $data['key'];
    ...
    foreach ($pids as $pid) {
        // Delete existing social account entries for this project and platform='tumblr'
        $del = $db->prepare("DELETE FROM social_accounts WHERE platform = 'tumblr' AND project_id = ?");
        $del->execute([$pid]);
        
        // Insert clean mapping
        $ins = $db->prepare("INSERT INTO social_accounts (user_id, project_id, platform, username, api_key, api_secret, password, status) VALUES (?, ?, 'tumblr', ?, ?, ?, ?, 'active')");
        $ins->execute([$uid, $pid, $blog, $apiKey, $apiSecret, $pass]);
        
        echo "✅ pd{$i} (User ID {$uid}) | Project #{$pid} => Mapped to Tumblr Blog #{$blogIndex}: '{$blog}'\n";
    }
}

echo "\n🎉 PERFECT 1-TO-1 USER-TO-TUMBLR BLOG MAPPING UPDATED IN DATABASE!\n";
